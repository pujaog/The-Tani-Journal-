...
// JSON storage used by the PostgreSQL-backed publishing workflow.
export async function createPost(accessToken, folderId, post) {
  const boundary = '-------tani-json-' + Math.random().toString(36).slice(2)
  const body = [
    `--${boundary}`,
    'Content-Type: application/json; charset=UTF-8',
    '',
    JSON.stringify({ name: `${post.id}.json`, parents: [folderId], mimeType: 'application/json' }),
    `--${boundary}`,
    'Content-Type: application/json; charset=UTF-8',
    '',
    JSON.stringify(post),
  ...
  return driveFetch(accessToken, `${DRIVE_UPLOAD}/files?uploadType=multipart&fields=id`, {
    method: 'POST',
    headers: { 'Content-Type': `multipart/related; boundary=${boundary}` },
    body,
  })
...
export async function updatePost(accessToken, folderId, post, fileId) {
  return driveFetch(accessToken, `${DRIVE_UPLOAD}/files/${fileId}?uploadType=media&fields=id`, {
    method: 'PATCH',
...
export async function readPost(accessToken, fileId) {
  return driveFetch(accessToken, `${DRIVE_API}/files/${fileId}?alt=media`)
}

export async function deletePost(accessToken, fileId) {
