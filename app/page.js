...
import {
  ...
  Video, Film, ArrowLeft, HardDrive, Cloud, CloudOff, RefreshCw, Bell, Shield, AlertCircle, Volume2, VolumeX
} from 'lucide-react'
import {
  ...
  if (media.type === 'video') {
    return (
      <div className={`${className} bg-black`}>
        <video src={media.url} controls playsInline preload="metadata" className="media-natural" />
      </div>
    )
  }
  return (
    <div className={className}>
      <img src={media.url} alt="" loading="lazy" decoding="async" className="media-natural" />
    </div>
  )
}
  ...
  const handleFile = async (e) => {
    ...
    for (const f of files) {
      ...
      if (f.size > 500 * 1024 * 1024) {
        setUploadErr(`"${f.name}" is over 500MB. Try a smaller file.`)
        continue
      }
      const dataUrl = await new Promise(res => {
        const r = new FileReader(); r.onload = () => res(r.result); r.readAsDataURL(f)
      })
      setImages(prev => [...prev, { url: dataUrl, aspectRatio: 'free', type: isVideo ? 'video' : 'image' }].slice(0, 6))
    }
    e.target.value = ''
  }
  const addSample = (url) => setImages(prev => [...prev, { url, aspectRatio: 'free', type: 'image' }].slice(0, 6))
        ...
        <div className="px-6 py-6 space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs journal-muted">up to 500MB each · natural size</span>
            </div>
...
function PostCard({ post, canEdit, authUser, presence, onEdit, onDelete, onLike, onReport, onOpenAuthor, onView, onCountChange }) {
  ...
  return (
    <article ref={cardRef} className="journal-card is-3d rounded-2xl border overflow-hidden fade-up">
      {first && (
...
function ProfileEditor({ open, onClose, profile, onSave }) {
  ...
  const pickPhoto = async (e) => {
    ...
    if (f.size > 500 * 1024 * 1024) return
...
// ---------- Author Profile Page ----------
function AuthorView({ uid, authUser, presence, onBack, onLike, onReport, onView, onCountChange, onMessage }) {
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {!isMe && (
                <div className="flex flex-wrap gap-2">
                  ...
                  <button onClick={() => onMessage?.(data.profile.uid, data.profile.displayName)} className="journal-chip inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border">
...
// ---------- Direct chat ----------
function ChatPanel({ open, onClose, authUser, recipientUid, recipientName }) {
  ...
  useEffect(() => {
    setSoundEnabled(localStorage.getItem('tani-chat-sound') === 'on')
  }, [])
  ...
  const loadMessages = useCallback(async (quiet = false) => {
    if (!open || !recipientUid) return
    if (!quiet) setLoading(true)
    try {
      const r = await authedFetch(`/api/messages?with=${encodeURIComponent(recipientUid)}`)
      const j = await r.json()
      const next = j.messages || []
      if (quiet && next.length > lastCount.current && next.some(m => m.senderUid !== authUser.uid)) playTone()
      lastCount.current = next.length
      setMessages(next)
      setError('')
    } catch (e) { if (!quiet) setError('Unable to load messages.') }
    finally { if (!quiet) setLoading(false) }
  }, [authUser.uid, open, playTone, recipientUid])

  useEffect(() => {
    if (!open) return
    lastCount.current = 0
    loadMessages()
    const iv = setInterval(() => loadMessages(true), 8000)
    return () => clearInterval(iv)
  }, [loadMessages, open])
  ...
  const send = async (event) => {
    ...
    if (!content || !recipientUid || sending) return
    setSending(true); setError('')
    try {
      const r = await authedFetch('/api/messages', { method: 'POST', body: JSON.stringify({ recipientUid, content }) })
      const j = await r.json()
  ...
  const toggleSound = () => {
    ...
    localStorage.setItem('tani-chat-sound', next ? 'on' : 'off')
    if (next) playTone()
  }

  if (!open) return null
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end p-0 sm:p-6 pointer-events-none">
      <div className="chat-panel journal-card is-3d pointer-events-auto w-full sm:w-[min(390px,calc(100vw-3rem))] h-[min(680px,100dvh)] sm:h-[min(680px,calc(100dvh-3rem))] sm:rounded-2xl border shadow-2xl flex flex-col overflow-hidden fade-up">
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: 'rgba(128,128,128,0.16)' }}>
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-sm font-semibold"><MessageCircle className="w-4 h-4" /> Messages</div>
            <div className="text-[11px] journal-muted truncate">{recipientUid ? `Chat with ${recipientName || 'journalist'}` : 'Open a profile to start a chat'}</div>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={toggleSound} className="journal-btn-ghost p-2 rounded-lg" title={soundEnabled ? 'Mute chat sounds' : 'Enable chat sounds'}>{soundEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}</button>
            <button onClick={onClose} className="journal-btn-ghost p-2 rounded-lg" title="Close messages"><X className="w-4 h-4" /></button>
          </div>
        </div>
        {!recipientUid ? (
          <div className="flex-1 flex items-center justify-center p-8 text-center text-sm journal-muted">Visit someone’s public profile and choose Message.</div>
        ) : (
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 chat-messages">
              {loading ? <div className="text-center text-xs journal-muted py-8">Loading conversation…</div> : messages.length === 0 ? <div className="text-center text-xs journal-muted py-8">A quiet beginning. Say hello.</div> : messages.map(message => (
                <div key={message.id} className={`flex ${message.senderUid === authUser.uid ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[82%] px-3 py-2 rounded-2xl text-sm ${message.senderUid === authUser.uid ? 'chat-bubble-own' : 'chat-bubble-other'}`}>
                    <div className="whitespace-pre-wrap break-words">{message.content}</div>
                    <div className="text-[10px] opacity-60 mt-1 text-right">{timeAgo(message.createdAt)}</div>
                  </div>
                </div>
              ))}
              <div ref={bottomRef} />
            ...
            <form onSubmit={send} className="p-3 border-t flex items-end gap-2" style={{ borderColor: 'rgba(128,128,128,0.16)' }}>
              <textarea value={draft} onChange={e => setDraft(e.target.value)} maxLength={2000} rows={1} placeholder="Write a message…" className="journal-input flex-1 px-3 py-2 rounded-xl text-sm resize-none" />
              <button type="submit" disabled={sending || !draft.trim()} className="journal-btn-primary p-2.5 rounded-xl disabled:opacity-40" title="Send message"><Send className="w-4 h-4" /></button>
            </form>
          </>
        )}
      </div>
...
function App() {
  ...
  const openChat = (uid, name) => {
    setChatTarget(uid ? { uid, name } : null)
    setChatOpen(true)
  }
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-30 backdrop-blur-md" style={{ background: 'color-mix(in srgb, currentColor 4%, transparent)' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => openChat()} className="journal-btn-ghost inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs" title="Open messages"><MessageCircle className="w-3.5 h-3.5" /><span className="hidden sm:inline">Chat</span></button>
            <div className="relative">
      ...
      <ChatPanel open={chatOpen} onClose={() => setChatOpen(false)} authUser={authUser} recipientUid={chatTarget?.uid} recipientName={chatTarget?.name} />
